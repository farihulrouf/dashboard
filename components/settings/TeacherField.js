...
export default function CustomizedHook(props) {
  const {course, setCourse, teachers} = props;
  const teacherIds = course.instructors.map((e)=>e._id)
  ...
  const {
    ...
    getOptionSelected: (option) => teacherIds.indexOf(option._id)>=0,
    ...
    <NoSsr>
      <div>
        ...
        {groupedOptions.length > 0 ? (
          <Listbox style={{width: '100%', paddingRight: 20}} {...getListboxProps()}>
            {groupedOptions.map((option, index) => (
              <li {...getOptionProps({ option, index })}>
                <img src={option.avatar} alt="Prof" width={50} height="auto" />
