...
export const updateTeacherApplication = async (appId, action) => {
    if(action === "accept"){
        const {data} = await axios.put(`/api/teacher-applications/${appId}/accept`)
        return data;
    }else if(action === "reject"){
        const {data} = await axios.post(`/api/teacher-applications/${appId}/reject`)
        return data;
    }
}
...
export const readMyNotification = async (notificationId) => {
    const {data} = await axios.put(`/api/users/me/notifications/${notificationId}`)
    return data;
}
