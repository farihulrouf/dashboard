...
export const getCourseRequests = async (courseId) => {
    const {data} = await axios.get(`/api/courses/${courseId}/requests`)
    return data;
}

export const acceptCourseRequest = async (courseId, userId) => {
    const {data} = await axios.put(`/api/courses/${courseId}/${userId}/acceptrequest`)
    return data;
}
